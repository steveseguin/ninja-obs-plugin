...
QString VDONinjaDock::buildUrl(bool push) const
{
    ...
    if (!rid.isEmpty()) url += "&room=" + QString::fromStdString(urlEncode(rid.toStdString()));
    if (!pass.isEmpty()) url += "&password=" + QString::fromStdString(urlEncode(pass.toStdString()));
...
void VDONinjaDock::updateStats()
{
    ...
    if (output) {
        ...
        if (vdo) {
            ...
            stats += QString("\n%1: %2 / %3")
                .arg(obs_module_text_vdo("VDONinja.Dock.Viewers"))
                .arg(viewers)
                .arg(maxV);
            ...
            if (tally.program) {
                ...
                lblTally->setStyleSheet(
                    "background: #00cc00; color: white; font-weight: bold; "
                    "border-radius: 8px; padding: 2px 8px; font-size: 12px;");
                lblTally->setVisible(true);
            } else {
                lblTally->setVisible(false);
