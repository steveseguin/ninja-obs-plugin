// Additional measured Chromium viewers for an already-running synthetic publisher.
...
async function main() {
  ...
  const report = { url, count, expectedFps, durationMs, reloads, viewers: [], ok: false };
  let browser;
  try {
    ...
    for (let round = 0; round <= reloads; round++) {
      ...
      await sleep(20000);
      await Promise.all(pages.map(page => startPresentationCapture(page, true, "counter-complement")));
      ...
      report.viewers.push(...captures.map((capture, index) => {
        ...
        const pixels = analyzeVisualSequence(capture.decodedRecords);
        return { index, round, samples: samples[index], capture, video, presentation, pixels,
          ok: video.ok && presentation.ok && pixels.ok };
      }));
    }
